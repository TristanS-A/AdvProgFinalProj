...
void handlePlayerMovement(Player* player, SDL_Rect &bgRect, const int SCREEN_WIDTH, const int SCREEN_HEIGHT, const Uint8* keystates){
    ...
    //Checks if the player is pressing a button to move
    if (keystates[SDL_SCANCODE_A] || keystates[SDL_SCANCODE_D] || keystates[SDL_SCANCODE_W] || keystates[SDL_SCANCODE_S]) {
        if (keystates[SDL_SCANCODE_A]) {
            ...
            if (bgRect.x < 0 && player->getPlayerRect().x == SCREEN_WIDTH / 2 - player->getPlayerRect().w / 2) {
                ...
                player->setPlayerVel(-player->getPlayerSpeed(), 0);
                ...
                if (player->getPlayerRect().x > 0) {
                    ...
                    else if (player->getPlayerRect().x < SCREEN_WIDTH / 2 - player->getPlayerRect().w / 2 &&
                             bgRect.x != 0) {
                        player->setPlayerPos(SCREEN_WIDTH / 2 - player->getPlayerRect().w / 2,
                                             player->getPlayerRect().y);
                    }
                }
            }
        }
        if ((keystates[SDL_SCANCODE_D])) {
            ...
            if (bgRect.x > -bgRect.w + SCREEN_WIDTH &&
                player->getPlayerRect().x == SCREEN_WIDTH / 2 - player->getPlayerRect().w / 2) {
                bgRect.x -= player->getPlayerSpeed();
                player->setPlayerVel(player->getPlayerSpeed(), 0);
        ...
        if (keystates[SDL_SCANCODE_W]) {
            ...
            if (bgRect.y < 0 && player->getPlayerRect().y == SCREEN_HEIGHT / 2 - player->getPlayerRect().h / 2) {
                ...
                player->setPlayerVel(0, -player->getPlayerSpeed());
                ...
                if (player->getPlayerRect().y > 0) {
                    ...
                    if (player->getPlayerRect().y < 0) {
                        ...
                        // the background instead of the player)
                    else if (player->getPlayerRect().y < SCREEN_HEIGHT / 2 - player->getPlayerRect().h / 2 &&
                             bgRect.y != 0) {
                        player->setPlayerPos(player->getPlayerRect().x,
        ...
        if ((keystates[SDL_SCANCODE_S])) {
            ...
            if (bgRect.y > -bgRect.h + SCREEN_HEIGHT &&
                player->getPlayerRect().y == SCREEN_HEIGHT / 2 - player->getPlayerRect().h / 2) {
                bgRect.y -= player->getPlayerSpeed();
                player->setPlayerVel(0, player->getPlayerSpeed());
        ...
        player->setPlayerVel(0, 0);
    }

    //Plays player walk animation
